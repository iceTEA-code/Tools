%
% out = plot_transect_continuous_rates(regress_out,ages_ka,transect_type,plot_ages,save_plot)
% out = plot_transect_continuous_rates(regress_out,ages_ka,transect_type,plot_ages,save_plot,mask)
% out = plot_transect_continuous_rates(regress_out,ages_ka,transect_type,plot_ages,save_plot,mask,time_lim,pos_lim,rate_lim)
%
% Plots the modelled regressions as the median with 95% confidence 
% bounds generated by either transect_regress_fourier.m or 
% transect_regress_spline.m. This is done separately for each nuclide. The 
% corresponding exposure ages are also plotted if plot_ages is set as 1.
%
% regress_out is a required struct of model rate information generated by 
% transect_regress_fourier.m or transect_regress_spline.m.
%
...
function out = plot_transect_continuous_rates(regress_out,ages_ka,transect_type,plot_ages,save_plot,mask,time_lim,pos_lim,rate_lim)

  % Check inputs
  if (nargin < 4 || nargin > 9)
      error('plot_transect_continuous_rates has wrong number of inputs!');
  end
...
%       end
      ...
      if isfield(N_rates_plot,'reg_rate68')
          ...
          legend(med,'Median (with 95% confidence)','Location','SouthWest'); legend('boxoff');
      end
      set(gca,'FontSize',10);
