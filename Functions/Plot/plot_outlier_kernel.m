%
% out = plot_outlier_kernel(all_ages_ka,new_ages_ka,plot_outliers,feature)
% out = plot_outlier_kernel(all_ages_ka,new_ages_ka,plot_outliers,feature,save_plot,mask)
% out = plot_outlier_kernel(all_ages_ka,new_ages_ka,plot_outliers,feature,save_plot,mask,x_lim,weighted)
%
% Plots kernel density estimates for exposure ages following identification
% of outliers, done separately for each nuclide. If specified, the outliers
% are additionally plotted in grey. The mode of the distribution, weighted 
% mean and standard deviation, and reduced chi-squared with critical value
% are computed for the dataset following removal of outliers if the ages 
% are from the same feature.
%
% all_ages_ka is a required struct, containing ages and uncertainties 
% calculated using age_calc.m or imported using get_ages.m, and a logical 
% of the nuclides that were measured.
%
% new_ages_ka is a required struct, containing ages and uncertainties 
% that is created following the removal of identified outliers using 
% find_outliers.m.
%
% plot_outliers should be a binary input of whether to additionally plot
% the removed outliers [1] or not [0].
%
...
function out = plot_outlier_kernel(all_ages_ka,new_ages_ka,plot_outliers,feature,save_plot,mask,x_lim,weighted)

  % Check inputs
  if (nargin < 4 || nargin > 8)
      error('plot_outlier_kernel has wrong number of inputs!');
  end
  if (nargin < 5 || isempty(save_plot))
      save_plot = 0;
  end  
  if (nargin < 6 || isempty(mask))
      mask = 1:length(new_ages_ka.sample_names);
  end
  if (nargin < 8 || isempty(weighted))
      weighted = 1;
  end
  
  if strcmpi(new_ages_ka.scaling_model,'SF')
      scaling = 'LSD';
  elseif strcmpi(new_ages_ka.scaling_model,'SA')
      scaling = 'LSDn';
  else
      scaling = new_ages_ka.scaling_model;
  end
  ...
  for a = 1:numel(NN_ind)
      ...
      if N_ind == 1
          ...
          ages_errs = new_ages_ka.Be10(mask,:);
          all_ages_errs = all_ages_ka.Be10(mask,:);
          new_idx = find(~isnan(new_ages_ka.Be10(:,1)));
          
      elseif N_ind == 2
          N_name = 'Al-26';
          col = [d_colours(2,:); l_colours(2,:)];
          ages_errs = new_ages_ka.Al26(mask,:);
          all_ages_errs = all_ages_ka.Al26(mask,:);
          new_idx = find(~isnan(new_ages_ka.Al26(:,1)));
      end
      ...
      if feature == 1
          ...
          all_ages = all_ages_errs(:,1);
          all_errs = all_ages_errs(:,2); % Use interior uncertainties
          ...
          ages = ages_errs(:,1);
          errs = ages_errs(:,2); % Use exterior uncertainties
          all_ages = all_ages_errs(:,1);
          all_errs = all_ages_errs(:,2); % Use exterior uncertainties
      end
      ...
      hold on;
      if plot_outliers == 1
          h_out = fancy_pants_camelplot(all_ages,all_errs);
          set(h_out(1:end-1),'Color',[.7,.7,.7],'Linewidth',1.5,'Linestyle','-');
          delete(h_out(new_idx));
          delete(h_out(end));
      end
      ...
      % Save figure
      if save_plot == 1
          fig_name = strcat(new_ages_ka.ages_name,'_ExposureAge_KDE_',N_name,'_',num2str(scaling));
