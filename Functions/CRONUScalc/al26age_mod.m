%
% [output,times,plotprodal,derivs]=al26age_mod(sampledata,sampleuncertainties,s
% caling_model)
%
...
%
% This is a modified version of CronusCalc's al26age.m. Uncertainties are
% calculated based only on elevation (pressure) and 26Al measurement 
% uncertainty.
%
% Modified by Richard Selwyn Jones, Durham University
% May 2018
%
...
% 3. Contemporary Elevation/latitude scaling factor for neutrons for Al (unitless)
...
% 8. Qs (unitless)
% 9. Qmu (unitless)
% 10. Inherited 26-Al (atoms/g of target)
% 11. Measured 26-Al (atoms/g of target)
...
%
%
function [output,times,plotprodal,derivs]=al26age_mod(sampledata,sampleuncertainties,scaling_model)
  expectedOutputs = 12;
  %
  ...
  %
  maxage=4248;      %6 half-lives = 4248 ka                       
  %
  ...
  if maxdepth > 2e5
    fprintf(1,'Maximum sample depth (%f) exceeds muon formulation of 2e5 g/cm2. \n Options: Lower the erosion rate, lower the maxage in file al26age, or change muon formulation in muonfluxsato.m',[maxdepth])
    warning('This sample exceeds muon maximum depth!');
    output=NaN*ones(expectedOutputs,1);
    [ times, plotprodal ] = getPlotData(maxage, sf, 0, 'al', scaling_model);
    return;
  end
  %
  % Computed parameters.
  ...
  %
  sf.currentsf=getcurrentsf(sf,0,scaling_model,'al');
  ...
  if (sp.concentration26 > 0.95*satN26)
    fprintf(1,'Saturated N26=%f, Measured N26=%f \n',[satN26; sp.concentration26]);
    warning('This sample is saturated!');
    output=NaN*ones(expectedOutputs,1);
    [ times, plotprodal ] = getPlotData(maxage, sf, ProdsAl, 'al', scaling_model);
    return;
  end
  %
  % Compute the nominal results.
  %
  output=al26ageraw(sampledata,pp,sf,scaling_model);
  age=output(1);
  
  [ times, plotprodal ] = getPlotData(age, sf, ProdsAl, 'al', scaling_model);
  ...
  % MODIFICATION: Only work through elevation (pressure) and measurement 
  % uncertainties.
  %
  uncert_mask = [4,10];
  
  derivs=zeros(length(uncert_mask),1);
  for i=1:length(uncert_mask)
      mask_idx = uncert_mask(i);
    if ((sampleuncertainties(mask_idx) ~= 0.0) | (nargout > 3))
      if (sampledata(mask_idx) ~= 0.0)
        thisdelta=0.01*abs(sampledata(mask_idx));
      else
        thisdelta=0.01;
      end
      deltasampledata=sampledata;
      deltasampledata(mask_idx)=deltasampledata(mask_idx)+thisdelta;
      deltasp=samppars1026(deltasampledata);
      deltasf=scalefacs1026(deltasp,scaling_model);
      deltaoutput=al26ageraw(deltasampledata,pp,deltasf,scaling_model);
      deltaage=deltaoutput(1);
      dagedi=(deltaage-age)/(thisdelta);
      derivs(mask_idx)=dagedi;
      if (~isnan(sampleuncertainties(mask_idx)))
        uncertainty=uncertainty+(dagedi^2*sampleuncertainties(mask_idx)^2);
      end
    end
  end
  %
  ...
  %
  deltapp=pp;
  deltapp.PsAl=pp.PsAl+0.01*abs(pp.PsAl);
  deltaoutput=al26ageraw(sampledata,deltapp,sf,scaling_model);
  deltaage=deltaoutput(1);
  dagedpsAl=(deltaage-age)/(0.01*abs(pp.PsAl));
  %This is the external uncertainty (total uncertainty)
  uncertaintyext=uncertainty+(dagedpsAl^2*pp.sigmaPsAl^2);
  %
  ...
  if size(output,1) ~= expectedOutputs
    error('Al outputs not expected size');
  end
end
